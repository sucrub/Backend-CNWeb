...
var _itempicture = require("./itempicture");
...
function initModels(sequelize) {
  var itempicture = _itempicture(sequelize, DataTypes);
  ...
  items.belongsToMany(orders, { as: 'order_id_orders', through: orderdetail, foreignKey: "item_id", otherKey: "order_id" });
  items.belongsToMany(tags, { as: 'tag_id_tags', through: tagitem, foreignKey: "item_id", otherKey: "tag_id" });
  items.belongsToMany(users, { as: 'user_id_users', through: rates, foreignKey: "item_id", otherKey: "user_id" });
  orders.belongsToMany(items, { as: 'item_id_items', through: orderdetail, foreignKey: "order_id", otherKey: "item_id" });
  tags.belongsToMany(items, { as: 'item_id_items_tagitems', through: tagitem, foreignKey: "tag_id", otherKey: "item_id" });
  ...
  itempicture.belongsTo(itemspecific, { as: "item_specific", foreignKey: "item_specific_id"});
  itemspecific.hasMany(itempicture, { as: "itempictures", foreignKey: "item_specific_id"});
